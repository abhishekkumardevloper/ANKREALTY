...
export default function Navbar() {
  ...
  const mainLinks = [
    { name: 'Buy', path: '/properties?category=buy', icon: Home },
    { name: 'Resale', path: '/properties?category=resale', icon: Key },
          ...
          <Link to="/" className="flex items-center group relative z-50">
            <img
              ...
              className={`h-10 md:h-12 w-auto object-contain transition-all duration-500 group-hover:scale-105 ${
                shouldBeSolid ? '' : 'brightness-0 invert drop-shadow-lg'
              }`}
            />
          </Link>
          ...
          <div className="hidden xl:flex items-center gap-8">
            ...
            <div className="relative group py-2">
              <button className={`flex items-center gap-1 text-xs font-black uppercase tracking-[0.15em] transition-colors ${
                shouldBeSolid ? 'text-slate-700 group-hover:text-[#8B0000]' : 'text-white/90 group-hover:text-white drop-shadow-md'
              }`}>
        ...
        <div className="h-full overflow-y-auto pb-32">
          <div className="p-6 space-y-2">
            ...
            {mainLinks.map((link) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isActive(link.path) ? 'bg-[#8B0000]/10 text-[#8B0000]' : 'bg-slate-100 text-slate-500'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {resourceLinks.map((item) => (
                <Link key={item.name} to={item.path} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col items-center justify-center gap-3 active:scale-95 transition-transform">
            ...
            <div className="mt-10 pt-8 border-t border-slate-100">
              {!user ? (
                ...
                <div className="space-y-3">
                  <Link to="/dashboard">
                    <Button variant="outline" className="w-full h-14 border-slate-200 text-slate-900 rounded-2xl text-base font-black flex items-center justify-center gap-3 bg-white">
