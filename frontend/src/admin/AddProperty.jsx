  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        <Button onClick={handleSubmit} className="bg-[#8B0000] hover:bg-[#600000] text-white font-bold h-11 px-6 rounded-xl shadow-md transition-colors">
      ...
      <form className="grid grid-cols-1 lg:grid-cols-3 gap-6" onSubmit={handleSubmit}>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div className="md:col-span-2">
                ...
                <Input name="title" placeholder="e.g. Luxury 4BHK Villa in Sector 150" value={form.title} onChange={handleChange} required className="focus:border-[#D4AF37]" />
              </div>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">Category</label>
                  <select name="category" value={form.category} onChange={handleChange} className="w-full h-10 px-3 border border-slate-200 rounded-md text-sm bg-white focus:border-[#D4AF37] outline-none">
                ...
                <div>
                  ...
                  <select name="property_type" value={form.property_type} onChange={handleChange} className="w-full h-10 px-3 border border-slate-200 rounded-md text-sm bg-white focus:border-[#D4AF37] outline-none">
              ...
              <div>
                ...
                <Input name="price" type="number" placeholder="e.g. 15000000" value={form.price} onChange={handleChange} required className="focus:border-[#D4AF37]" />
              </div>

              <div className="grid grid-cols-3 gap-3 md:col-span-2">
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">Area (Sq.ft)</label>
                  <Input name="area" type="number" placeholder="Area" value={form.area} onChange={handleChange} className="focus:border-[#D4AF37]"/>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">BHK</label>
                  <Input name="bhk" type="number" placeholder="BHK" value={form.bhk} onChange={handleChange} className="focus:border-[#D4AF37]"/>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">Bathrooms</label>
                  <Input name="bathrooms" type="number" placeholder="Baths" value={form.bathrooms} onChange={handleChange} className="focus:border-[#D4AF37]"/>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea name="description" rows={5} placeholder="Detailed property description..." value={form.description} onChange={handleChange} className="focus:border-[#D4AF37]" />
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div><label className="text-xs font-bold text-slate-700 mb-1 block">City *</label><Input name="city" value={form.city} onChange={handleChange} required className="focus:border-[#D4AF37]"/></div>
              <div><label className="text-xs font-bold text-slate-700 mb-1 block">State</label><Input name="state" value={form.state} onChange={handleChange} className="focus:border-[#D4AF37]"/></div>
              <div className="md:col-span-2"><label className="text-xs font-bold text-slate-700 mb-1 block">Location / Sector *</label><Input name="location" value={form.location} onChange={handleChange} required className="focus:border-[#D4AF37]"/></div>
              
              <div><label className="text-xs font-bold text-slate-700 mb-1 block">Builder Name</label><Input name="builder" value={form.builder} onChange={handleChange} placeholder="e.g. Yatharth Group" className="focus:border-[#D4AF37]"/></div>
              <div><label className="text-xs font-bold text-slate-700 mb-1 block">RERA Number</label><Input name="rera" value={form.rera} onChange={handleChange} placeholder="e.g. UPRERAPRJ12345" className="focus:border-[#D4AF37]"/></div>
              
              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">Project Status</label>
                <select name="projectStatus" value={form.projectStatus} onChange={handleChange} className="w-full h-10 px-3 border border-slate-200 rounded-md text-sm bg-white focus:border-[#D4AF37] outline-none">
                  <option value="New Launch">New Launch</option>
                  <option value="Under Construction">Under Construction</option>
                  <option value="Ready to Move">Ready to Move</option>
                </select>
              </div>
              <div><label className="text-xs font-bold text-slate-700 mb-1 block">Possession Date</label><Input name="possession" value={form.possession} onChange={handleChange} placeholder="e.g. Dec 2026" className="focus:border-[#D4AF37]"/></div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6 sticky top-24">
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:bg-slate-50 hover:border-[#D4AF37] transition-colors">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                ...
                {media.images.map((img, idx) => (
                  <div key={`new-${idx}`} className="relative w-16 h-16 bg-slate-100 rounded-lg overflow-hidden border-2 border-[#D4AF37]">
                    ...
                    <button type="button" onClick={() => removeMedia(idx, 'images')} className="absolute top-1 right-1 bg-white/90 text-red-600 rounded-full p-0.5 hover:bg-red-600 hover:text-white transition-colors shadow-sm"><X className="w-3 h-3" /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded uppercase tracking-wider">{media.videos.length + media.existingVideos.length} / 2 Max</span>
              </div>
              <label className="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:bg-slate-50 hover:border-[#D4AF37] transition-colors">
            ...
            <div>
              ...
              <label className={`flex flex-col items-center justify-center w-full h-20 border-2 ${(media.pdf || media.existingPdf) ? 'border-emerald-500 bg-emerald-50' : 'border-dashed border-slate-300 hover:bg-slate-50 hover:border-[#D4AF37]'} rounded-xl cursor-pointer transition-colors`}>
                <FileText className={`w-5 h-5 mb-1 ${(media.pdf || media.existingPdf) ? 'text-emerald-600' : 'text-slate-400'}`} />
                <span className={`text-xs font-medium ${(media.pdf || media.existingPdf) ? 'text-emerald-700' : 'text-slate-500'}`}>
                  {(media.pdf || media.existingPdf) ? 'Brochure Attached' : 'Upload PDF Document'}
                </span>
              ...
              {media.existingPdf && !media.pdf && (
                <div className="flex items-center justify-between mt-2 px-1">
                  <span className="text-xs text-slate-600 truncate max-w-[200px]">Existing Brochure.pdf</span>
                  <button type="button" onClick={() => removeMedia(0, 'existingPdf')} className="text-xs text-red-600 font-bold hover:underline">Remove</button>
                </div>
              )}
              {media.pdf && (
                <div className="flex items-center justify-between mt-2 px-1">
