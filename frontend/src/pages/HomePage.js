...
import { 
  ...
  TrendingUp, Award, ShieldCheck, Home, Key, PieChart
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-[#D4AF37]/30 relative">
      ...
      <section className="relative pt-32 pb-28 px-4 md:px-6 overflow-hidden min-h-[90vh] flex flex-col justify-center">
        ...
        <div className="relative z-20 max-w-6xl mx-auto text-center mt-10 w-full">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="space-y-6">
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 mb-2 px-5 py-2 rounded-full border border-[#D4AF37]/40 bg-[#D4AF37]/10 backdrop-blur-sm text-[#D4AF37] text-xs md:text-sm font-bold tracking-widest uppercase shadow-[0_0_15px_rgba(212,175,55,0.2)]">
              <Award className="w-4 h-4" /> Your Trusted Partner
            </motion.div>
          ...
          {/* Search Box */}
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.6 }} className="mt-12 bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl p-4 md:p-6 max-w-5xl mx-auto border border-[#D4AF37]/20 text-left relative z-30">
            <div className="flex flex-wrap justify-center md:justify-start gap-3 mb-6 px-2 border-b border-slate-200 pb-4">
              {categoryOptions.map((cat) => (
                <button key={cat.value} onClick={() => setSearch((prev) => ({ ...prev, category: cat.value }))} className={`px-6 py-2.5 rounded-full font-bold transition-all ${search.category === cat.value ? 'bg-[#8B0000] text-white shadow-md' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}>{cat.label}</button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 relative">
                  ...
                  <div className="absolute left-0 right-0 top-full mt-4 bg-white border border-slate-100 rounded-2xl shadow-2xl p-2 z-50 overflow-hidden">
                    {suggestions.slice(0, 5).map((item) => (
                      <button key={item.name} type="button" onClick={() => { setSearch((prev) => ({ ...prev, city: item.city, property_type: item.propertyType })); setSearchFocused(false); }} className="w-full text-left px-4 py-3 rounded-xl hover:bg-slate-50 transition-colors flex justify-between items-center group/item">
                        <div>
                          <p className="font-bold text-slate-900 group-hover/item:text-[#8B0000] transition-colors">{item.name}</p>
              ...
              <Button onClick={handleSearch} className="w-full h-14 bg-[#8B0000] hover:bg-[#600000] hover:-translate-y-1 transition-all duration-300 text-white font-black text-lg rounded-xl shadow-lg shadow-[#8B0000]/30">
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <p className="text-[#8B0000] font-bold uppercase tracking-[0.25em] text-xs mb-3">Excellence in Real Estate</p>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900">Comprehensive Property Solutions</h2>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="bg-white p-8 rounded-[2rem] border border-slate-200 hover:border-[#D4AF37] hover:shadow-xl transition-all duration-300 group cursor-pointer">
                <div className="w-14 h-14 bg-slate-50 rounded-full flex items-center justify-center mb-6 group-hover:bg-[#8B0000] transition-colors duration-300">
                ...
                <div className="flex items-center text-[#8B0000] font-bold group-hover:text-[#D4AF37] transition-colors">
      ...
      <section className="py-24 px-6 bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            <Link to="/properties">
              <Button variant="outline" className="border-slate-300 font-bold hover:bg-[#8B0000] hover:text-white transition-colors h-12 px-6 rounded-xl">View all properties <ChevronRight className="w-4 h-4 ml-2" /></Button>
            </Link>
          </motion.div>
      ...
      <section className="py-24 px-6 bg-slate-900 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#D4AF37]/10 rounded-full blur-3xl pointer-events-none -translate-y-1/2 translate-x-1/3" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#8B0000]/20 rounded-full blur-3xl pointer-events-none translate-y-1/2 -translate-x-1/3" />
        
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
             <p className="text-[#D4AF37] font-bold uppercase tracking-[0.25em] text-xs mb-3">Financial Planning</p>
             <h2 className="text-3xl md:text-5xl font-black text-white">Smart EMI Calculator</h2>
             <p className="text-slate-400 mt-4 max-w-2xl mx-auto">Plan your property purchase with confidence. Estimate your monthly mortgage payments instantly.</p>
          </div>

          <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl max-w-5xl mx-auto grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-xl font-black text-[#8B0000]">{formatCurrency(loanAmount)}</span>
                </div>
                <input 
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-xl font-black text-[#8B0000]">{interestRate}%</span>
                </div>
                <input 
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-xl font-black text-[#8B0000]">{loanTenure} Years</span>
                </div>
                <input 
            ...
            <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 text-center relative overflow-hidden">
               ...
               <PieChart className="w-12 h-12 text-[#D4AF37] mx-auto mb-6" />
               <p className="text-slate-500 font-bold uppercase tracking-wider text-sm mb-2">Equated Monthly Installment (EMI)</p>
               <h3 className="text-5xl font-black text-[#8B0000] mb-6">{formatCurrency(calculateEMI())}</h3>
      ...
      <section className="py-24 px-6 bg-slate-50 border-b border-slate-100 relative overflow-hidden">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center relative z-10">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            <p className="text-[#8B0000] font-bold uppercase tracking-[0.25em] text-xs mb-3">Investment Hub</p>
            <h2 className="text-3xl md:text-5xl font-black mb-6 leading-tight text-slate-900">Why buyers choose India’s growth markets</h2>
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {[
                ...
                <div key={idx} className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-[#D4AF37]/50 transition-all">
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="bg-[#8B0000] text-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#D4AF37]/20 rounded-full blur-3xl pointer-events-none -translate-y-1/2 translate-x-1/2" />
              ...
              {bankOffers.map((offer) => (
                <div key={offer.bank} className="p-5 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between gap-4 hover:bg-white/10 transition-colors">
                  <div>
                    ...
                    <p className="text-[#D4AF37]/70 text-sm mt-0.5">{offer.note}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[#D4AF37] font-black text-xl">{offer.rate}</p>
                    <p className="text-xs text-white/50 uppercase tracking-wider mt-1">Indicative</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-6 relative z-10">
              <Input value={loanLead.name} onChange={(e) => setLoanLead((prev) => ({ ...prev, name: e.target.value }))} placeholder="Your full name" className="bg-white/10 border-white/20 text-white placeholder:text-white/50 h-14 rounded-xl focus:border-[#D4AF37]" />
              <Input value={loanLead.phone} onChange={(e) => setLoanLead((prev) => ({ ...prev, phone: e.target.value }))} placeholder="Phone number" className="bg-white/10 border-white/20 text-white placeholder:text-white/50 h-14 rounded-xl focus:border-[#D4AF37]" />
            </div>
            
            <div className="flex flex-col sm:flex-row gap-4 relative z-10">
              <Button onClick={handleLoanLead} className="bg-[#D4AF37] hover:bg-[#c09b2e] text-slate-900 h-14 rounded-xl text-base px-8 font-black flex-1 shadow-lg shadow-[#D4AF37]/20 transition-colors">Request callback</Button>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 bg-slate-900 text-white relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div className="bg-[#D4AF37]/20 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-8 backdrop-blur-sm border border-[#D4AF37]/50 shadow-[0_0_20px_rgba(212,175,55,0.3)]">
          ...
          <p className="text-slate-300 text-lg md:text-xl mb-12 max-w-2xl mx-auto">Get pre-launch alerts, price updates, and curated luxury property matches directly to your inbox and WhatsApp.</p>
          
          <div className="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
            <input type="email" value={newsletterEmail} onChange={(e) => setNewsletterEmail(e.target.value)} placeholder="Enter your email address" className="flex-1 h-16 rounded-2xl px-6 bg-white/10 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37] transition-all text-lg" />
            <Button onClick={handleNewsletter} className="h-16 px-10 bg-[#D4AF37] hover:bg-[#c09b2e] text-slate-900 font-black rounded-2xl text-lg shadow-xl shadow-[#D4AF37]/20">Subscribe Now</Button>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
            ...
            <div>
              ...
              <ul className="space-y-5 text-slate-400 font-medium">
                <li><Link to="/properties" className="hover:text-[#D4AF37] transition-colors flex items-center"><ChevronRight className="w-4 h-4 mr-2 text-[#8B0000]"/> All Properties</Link></li>
                ...
                <li><Link to="/contact" className="hover:text-[#D4AF37] transition-colors flex items-center"><ChevronRight className="w-4 h-4 mr-2 text-[#8B0000]"/> Contact Support</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-5 text-slate-400 font-medium">
                <li><Link to="/properties?property_type=plot" className="hover:text-[#D4AF37] transition-colors flex items-center"><ChevronRight className="w-4 h-4 mr-2 text-[#8B0000]"/> Premium Plots</Link></li>
                <li><Link to="/buy" className="hover:text-[#D4AF37] transition-colors flex items-center"><ChevronRight className="w-4 h-4 mr-2 text-[#8B0000]"/> Residential Properties</Link></li>
                <li><Link to="/properties?property_type=commercial" className="hover:text-[#D4AF37] transition-colors flex items-center"><ChevronRight className="w-4 h-4 mr-2 text-[#8B0000]"/> Commercial Spaces</Link></li>
                <li><Link to="/rent" className="hover:text-[#D4AF37] transition-colors flex items-center"><ChevronRight className="w-4 h-4 mr-2 text-[#8B0000]"/> Rental Homes</Link></li>
              </ul>
            </div>
