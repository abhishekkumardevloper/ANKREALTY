...
import { 
  ...
  CheckCircle, Facebook, Twitter, Instagram, Linkedin
} from 'lucide-react';

// --- DUMMY CLIENT LOGOS / NAMES ---
// Replace the 'src' with your actual client logo image paths.
const clients = [
...
const industries = [
  { name: "IT & Technology", icon: Monitor, desc: "Tech parks and smart offices for software giants and startups." },
  { name: "Banking & Finance", icon: Landmark, desc: "Secure, premium spaces in prime business districts." },
  { name: "Consulting Firms", icon: Briefcase, desc: "Grade-A corporate towers for global advisory networks." },
  { name: "Retail & E-commerce", icon: Building2, desc: "High-visibility retail hubs and warehousing logistics." },
  { name: "MNCs & Foreign Ops", icon: Globe, desc: "Compliance-ready spaces for multinational expansions." },
  { name: "Advertising & Media", icon: FileText, desc: "Creative, open-plan environments for publishing houses." }
];
...
export default function ClientsPage() {
  ...
  const handleLeadSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      toast.success("Corporate inquiry sent successfully! Our leasing expert will contact you.");
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-[#D4AF37]/30">
      ...
      {/* --- HERO SECTION --- */}
      <section className="bg-[#050505] text-white pt-32 pb-24 px-6 text-center relative overflow-hidden">
      ...
      <section className="relative z-20 -mt-10 max-w-6xl mx-auto px-6">
        <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-slate-100">
          {[
            ...
            <div key={i} className="text-center px-4 group">
              <h3 className="text-4xl md:text-5xl font-black text-slate-900 group-hover:text-[#8B0000] transition-colors">{stat.value}</h3>
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-slate-900">Trusted by Global Leaders</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {clients.map((client, index) => (
              <div key={index} className="bg-white h-32 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-center p-6 hover:border-[#D4AF37] hover:shadow-lg transition-all duration-300 group grayscale hover:grayscale-0">
                <img 
                  src={client.src} 
          ...
          <div className="text-center mt-12">
            <p className="text-slate-400 font-medium italic">...and hundreds of other dynamic enterprises and startups.</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {industries.map((ind, i) => (
              <div key={i} className="bg-white/5 border border-white/10 p-8 rounded-3xl hover:bg-white/10 hover:border-[#D4AF37]/50 transition-all duration-300 group">
                <div className="w-14 h-14 bg-[#D4AF37]/10 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-[#8B0000] transition-colors border border-[#D4AF37]/30">
      ...
      <section className="py-24 px-6 bg-slate-50 border-b border-slate-200">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          {/* Download & Info */}
          <div>
            <p className="text-[#8B0000] font-bold uppercase tracking-[0.25em] text-xs mb-3">Corporate Portfolio</p>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6 hover:border-[#D4AF37]/50 transition-colors">
               <div>
                 <h4 className="font-black text-slate-900 mb-1">Our Business Profile</h4>
                 <p className="text-sm text-slate-500">Download our comprehensive company presentation.</p>
               </div>
               <Button className="shrink-0 bg-[#8B0000] hover:bg-[#600000] text-white font-bold h-12 px-6 rounded-xl shadow-lg shadow-[#8B0000]/30 transition-all">
          ...
          <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-2xl border border-slate-100 relative overflow-hidden">
            ...
            <p className="text-slate-500 text-sm mb-8">Fill out the form below and our corporate leasing team will get back to you promptly.</p>
            ...
            <form onSubmit={handleLeadSubmit} className="space-y-5 relative z-10">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <Input 
                    required placeholder="e.g. Tech Corp"
              ...
              <div>
                ...
                <Textarea 
                  ...
                  className="bg-slate-50 border-slate-200 focus:border-[#D4AF37] rounded-xl font-medium resize-none p-4"
                />
              </div>
              ...
              <Button type="submit" disabled={isSubmitting} className="w-full h-14 bg-[#D4AF37] hover:bg-[#c09b2e] text-slate-900 font-black rounded-xl text-base shadow-lg shadow-[#D4AF37]/30 transition-all hover:-translate-y-0.5 mt-4">
                {isSubmitting ? "Submitting Request..." : "Request Leasing Consultation"} <ArrowRight className="w-5 h-5 ml-2" />
              </Button>
            </form>
          </div>
        </div>
      ...
      <section className="bg-[#8B0000] py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-white text-center md:text-left">
           <div>
              ...
              <p className="text-white/80 text-sm">Our corporate advisory team is available 24/7.</p>
           </div>
           <div className="flex flex-col sm:flex-row gap-4">
              <a href="tel:+919732300007" className="flex items-center bg-white/10 hover:bg-white/20 border border-white/20 px-6 py-3 rounded-xl font-bold transition-colors">
                 <Phone className="w-5 h-5 mr-3 text-[#D4AF37]" /> +91 97323 00007
              </a>
              <a href="mailto:contact@ankrealty.com" className="flex items-center bg-white/10 hover:bg-white/20 border border-white/20 px-6 py-3 rounded-xl font-bold transition-colors">
